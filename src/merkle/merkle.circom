...
template Merkle (levels) {
    signal input step_in[3]; // modelHash, dataMerkleRoot, outMerkleRoot
    ...
    signal input newDataRoot;
    signal input newOutRoot;
    signal input pathIndices;
    signal input dataPathElements[levels];
    signal input outPathElements[levels];
    ...
    dataTree.oldRoot <== step_in[1];
    dataTree.newRoot <== newDataRoot;
    dataTree.leaf <== cid_out[0]; // TODO: should use both outputs
    dataTree.pathIndices <== pathIndices;
    ...
    outTree.newRoot <== newOutRoot;
    outTree.leaf <== model_out;
    outTree.pathIndices <== pathIndices;
    for (var i = 0; i < levels; i++) {
        outTree.pathElements[i] <== outPathElements[i];
    }
